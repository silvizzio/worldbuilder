---
title: WebUI 开放 API 参考
description: 面向外部 WebUI、调试工具和业务系统集成的开放 API 参考，覆盖场景控制、相机、模型、POI 和界面图层等能力。
---
...
本文档说明 WORLD BUILDER Viewer 与外部 WebUI 之间的开放通信能力，适用于在数字孪生场景中接入自定义业务面板、调试控制台或上层业务系统。当前版本覆盖已上线的 V1 能力，集成方可通过标准方法控制场景、相机、模型、POI 和界面图层。
...
|------|------------|----------------|----------|
| **Guest WebUI** | 嵌入 Viewer 的用户自定义页面（iframe） | `window.parent` | 通过业务面板控制相机、模型、POI 和界面图层 |
| **Host 调试页** | Playground 等外层页面 | `iframe.contentWindow`（Viewer） | 用于接口联调、交付验收和示例代码生成 |

集成约束：

- 仅通过浏览器 `window.postMessage` 通信，**禁止**将 `targetOrigin` 设置为 `*`。
- Guest 侧调用为**会话级运行时效果**：模型和 POI 显示/隐藏等运行时修改不会写入项目数据库；刷新 Viewer 后会恢复为项目配置中的状态。
...
建议集成方优先使用官方 SDK 接入开放 API。SDK 已封装握手、请求响应、事件监听和超时处理逻辑，可降低直接处理 `postMessage` 协议的接入复杂度。
...
| `client.onEvent(handler): () => void` | 监听 Host 推送事件，并返回取消监听函数 |
...
所有消息均采用统一 JSON Envelope，便于 Host 与 Guest 在同一协议通道内区分请求、响应和事件：
...
开放 API 遵循明确的会话生命周期。Guest 应在完成握手并收到场景加载事件后调用业务 API，避免在 Viewer 尚未就绪时触发 `NOT_READY`。
...
| `model.show` | 显示模型（可独占） |
| `model.hide` | 隐藏模型 |
| `model.setVisibility` | 批量设置可见状态 |
| `model.getVisibility` | 查询可见状态 |
| `poi.list` | 列出 POI |
| `poi.setVisible` | 设置 POI 可见状态（会话级） |
...
建立开放 API 会话。除异常场景外，这是唯一可在场景未加载时正常完成的方法。
...
| `visible` | 综合项目配置与会话级运行时控制后的有效可见状态 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `ids` | `string[]` | 否 | 省略时返回当前场景全部模型的可见状态 |
...
显示和隐藏操作作用于运行时界面图层列表，与编辑器**界面**面板中的页面管理保持一致。

---

## 7. 事件（Host → Guest）

V1 当前已实现以下由 Host 推送到 Guest 的单向事件：
...
| code | 含义 |
|------|------|
| `FORBIDDEN` | origin 不被允许，或方法不在白名单中 |
| `NOT_READY` | 场景或相机尚未就绪 |
| `INVALID_PARAMS` | 参数缺失或格式错误 |
| `NOT_FOUND` | 资源不存在，例如 POI |
...
- **Origin 校验**：当前 Host 仅允许与 Viewer 同源的 `event.origin`；如需接入跨域 Guest，需在 Host 侧扩展 `isOriginAllowed`（见 `open-bridge.js` / `initTwinScapeOpenBridgeHost`）。
...
- **无持久化写入接口**：Guest 不能通过开放 API 修改项目数据库，所有运行时控制均限定在当前 Viewer 会话内。
...
以下文件用于 API 实现、能力注入和调试验证。排查集成问题时，建议优先确认 Host 桥接、SDK 版本与 Playground API 列表是否保持一致。

| 文件 | 说明 |
|------|------|
| `open-bridge.js` | Host 消息路由、方法分发和事件广播 |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-06-30 | 1.0 | 初版：对齐 `ALLOWED_METHODS` 共 18 个方法；事件仅包含 `scene.loaded`；补充 SDK、协议和会话级语义说明 |

**后续修改开放 API 时，请同步更新上表，并检查 `playground.html` 内 `API_CATALOG` 是否与本文档一致。**
